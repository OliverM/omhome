[:article
 ...
 [:section
  ...
  [:p "If none of those words make any sense to you, Emacs and Vim are text editing tools, in use for decades, by technically adept people who feel that keyboard-orientated interfaces are more efficient than mouse- or touch-driven ones. Vim takes this further than Emacs, eschewing the use of the mouse entirely once you've accumulated enough expertise. Emacs prides itself more on being " [:emphasis "programmable,"] " by which it means infinitely adaptable to your needs, assuming you're prepared to master its conventions, both UI-wise and how it's programmed in the first place. Vim is also extremely adaptable, but programming it to do your bidding is more demanding, and in general it's less amenable to change than Emacs." ]
  [:p "I've watched "[:a {:href "https://vimeo.com/53144573"} "Drew Neil's talk on Vim"] " and bought his book, which I'm still working through. I've run through the VimTutor more than once, in Vim and in Spacemacs, and I've used Spacemacs for real work (both client work and to build the static site generator that creates these pages)."
   [:ul
    ...
    [:li "Inadvertent deletions, part two: The delete key 'd' is also the second half of the Spacemacs escape-analogue 'fd' which cancels the current mode or action, returning you to command mode. Unless you're in neotree, in which case it ignores the f, interprets the d as delete, and pops up a small are-you-sure y/n prompt - so if your next task is to " [:b "y"]"ank something... poof!"]]]]]
